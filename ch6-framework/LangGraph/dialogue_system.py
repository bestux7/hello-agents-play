"""
三步问答-智能搜索助手，基于LangGraph + Tavily API的真实搜索系统
1. 理解用户需求
...
class SearchState(TypedDict):
  ...
  user_query: str       # 优化后的用户查询
  search_query: str     # 优化后用于tavily的查询
...
def understand_query_node(state: SearchState) -> SearchState:
  ...
  elif "搜索关键字：" in response_txt:
    search_query = response_txt.split("搜索关键字：")[1].strip()
