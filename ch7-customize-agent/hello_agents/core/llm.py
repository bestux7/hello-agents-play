...
class HelloAgentsLLM:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # 优先使用指定的provider，否则自动检测
        self.provider = provider or self._auto_detect_provider(api_key, base_url)
        # 根据provider确定api_key和base_url
        self.api_key, self.base_url = self._resolve_credential(api_key, base_url)

        # 验证必要参数model和api_key/base_url存在
        if not self.model:
    ...
    def _resolve_credential(self, api_key: Optional[str], base_url: Optional[str]) -> tuple[str, str]:
    ...
    def think(self, messages: List[Dict[str, str]], temperature: Optional[float] = None) -> Iterator[str]:
        ...
        try:
            ...
            for chunk in response:
                # 有些事件（比如 usage / 结束事件）choices 可能是 []
