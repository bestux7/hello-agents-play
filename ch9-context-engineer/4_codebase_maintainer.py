...
from hello_agents.tools import NoteTool,  MemoryTool, TerminalTool
...
class CodebaseMaintainer:
    ...
    - 跨会话记忆和上下文管理
    """
    def __init__(self, project_name: str, codebase_path: str, llm: Optional[HelloAgentsLLM]):
    ...
    def _build_base_system_prompt(self) -> str:
        ...
        ⚠️ 重要：当前运行环境是 Windows，请始终使用 Windows 命令：
        - 列目录：dir（不是 ls）
        - 查看文件内容：type filename（不是 cat）
        - 搜索文本：findstr "keyword" filename（不是 grep）
        - 查找文件：dir /s /b *.py（不是 find）
        - 目录树：tree /f（不是 tree 本身也可以用）
    ...
    def _notes_to_packets(self, notes: List[Dict]) -> List[ContextPacket]:
        ...
        for note in notes:
            ...
            relevance = relavance_map.get(note_type, 0.6)
    ...
    def _build_system_instructions(self, mode: str="auto") -> str:
        """构建系统指令（Agentic 方式）"""
...
def main():
    ...
    maintainer = CodebaseMaintainer(
        project_name="my_agent_demo",
        codebase_path="./ch1-know-agent",
        llm=HelloAgentsLLM()
    )
